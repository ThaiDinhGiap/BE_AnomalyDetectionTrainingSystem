...
public interface TrainingResultDetailRepository extends JpaRepository<TrainingResultDetail, Long> {
        ...
        long countByFilters(@Param("createdBy") String createdBy, @Param("teamId") Long teamId,
                        @Param("lineId") Long lineId, @Param("year") Integer year);
        ...
        @Query("SELECT count(d) FROM TrainingResultDetail d JOIN d.trainingResult r " +
                        ...
                        "AND (:lineId IS NULL OR r.line.id = :lineId) " +
                        "AND (:year IS NULL OR r.year = :year)")
        long countByFiltersAndIsPass(@Param("createdBy") String createdBy, @Param("teamId") Long teamId,
                        @Param("lineId") Long lineId, @Param("year") Integer year, @Param("isPass") boolean isPass);

        @Query("SELECT count(d) FROM TrainingResultDetail d JOIN d.trainingResult r " +
                        "WHERE d.actualDate IS NOT NULL " +
                        "AND (d.signatureFiIn.id = :fiUserId OR d.signatureFiOut.id = :fiUserId) " +
                        "AND d.deleteFlag = false " +
                        "AND (:teamId IS NULL OR r.team.id = :teamId) " +
                        "AND (:lineId IS NULL OR r.line.id = :lineId) " +
                        "AND (:year IS NULL OR r.year = :year)")
        long countByFiAndFilters(@Param("fiUserId") Long fiUserId, @Param("teamId") Long teamId,
                        @Param("lineId") Long lineId, @Param("year") Integer year);
        ...
        @Query("SELECT count(d) FROM TrainingResultDetail d JOIN d.trainingResult r " +
                        ...
                        "AND (:lineId IS NULL OR r.line.id = :lineId) " +
                        "AND (:year IS NULL OR r.year = :year)")
        long countByFiAndFiltersAndIsPass(@Param("fiUserId") Long fiUserId, @Param("teamId") Long teamId,
                        @Param("lineId") Long lineId, @Param("year") Integer year, @Param("isPass") boolean isPass);

        @Query("SELECT count(d) FROM TrainingResultDetail d JOIN d.trainingResult r " +
                        "WHERE d.actualDate IS NOT NULL " +
                        "AND (r.team.group.id IN :groupIds) " +
                        "AND (:teamId IS NULL OR r.team.id = :teamId) " +
                        "AND (:lineId IS NULL OR r.line.id = :lineId) " +
                        "AND (:year IS NULL OR r.year = :year)")
        long countByGroupsAndFilters(@Param("groupIds") List<Long> groupIds, @Param("teamId") Long teamId,
                        @Param("lineId") Long lineId, @Param("year") Integer year);
        ...
        @Query("SELECT count(d) FROM TrainingResultDetail d JOIN d.trainingResult r " +
                        ...
                        "AND (:lineId IS NULL OR r.line.id = :lineId) " +
                        "AND (:year IS NULL OR r.year = :year)")
        long countByGroupsAndFiltersAndIsPass(@Param("groupIds") List<Long> groupIds, @Param("teamId") Long teamId,
                        @Param("lineId") Long lineId, @Param("year") Integer year, @Param("isPass") boolean isPass);
        ...
        // FI: details PENDING_CONFIRMATION assigned to a specific FI user
        @Query("""
                    SELECT d FROM TrainingResultDetail d
                    WHERE d.status = com.sep490.anomaly_training_backend.enums.ReportStatus.PENDING_CONFIRMATION
                      AND (d.signatureFiIn.id = :fiUserId OR d.signatureFiOut.id = :fiUserId)
                      AND (d.fiInConfirmed IS NULL OR d.fiOutConfirmed IS NULL)
                      AND d.deleteFlag = false
                """)
        List<TrainingResultDetail> findPendingFiConfirmationByAssignedFi(@Param("fiUserId") Long fiUserId);
        ...
        @Query("""
                            SELECT DISTINCT d.trainingResult.id FROM TrainingResultDetail d
                            WHERE (d.signatureFiIn.id = :fiUserId OR d.signatureFiOut.id = :fiUserId)
                              AND d.deleteFlag = false
                        """)
        List<Long> findResultIdsByAssignedFi(@Param("fiUserId") Long fiUserId);
}
